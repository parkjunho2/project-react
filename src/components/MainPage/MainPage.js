...
const MainPage = () => {
    ...
    //callback
    const changeInputString = useCallback(
        (e) => {
            setInput({
                ...input,
    ...
    useEffect(() => {
        setInput((prev) => ({
            ...prev,
            departureAirport: selectedDepCity
                ? selectedDepCity
                : prev.departureAirport, // 선택된 도시가 있으면 해당 도시로 설정
        }));
        ...
        (e) => {
            ...
            if (e.key === 'ArrowDown') {
                ...
                setSelectedIndex((prevIndex) =>
                    prevIndex < searchResult.length - 1
                        ? prevIndex + 1
                        : prevIndex
                );
    ...
    const getWeatherData = useCallback(async (latitude, longitude) => {
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current_weather=true&hourly=temperature_2m&daily=temperature_2m_max,temperature_2m_min,weathercode&timezone=Asia%2FSeoul&start_date=${moment()
            .utcOffset(9)
    ...
    useEffect(() => {
        ...
        const countryCoordinates = {
            ...
            krw: { latitude: 37.5665, longitude: 126.978 }, // 한국 서울
        };
    ...
    useEffect(() => {
        const modalClosedTime = window.localStorage.getItem('modalClosedTime');
        const currentTime = new Date().getTime();

        if (
            !modalClosedTime ||
            currentTime - modalClosedTime > 24 * 60 * 60 * 1000
        ) {
            setIsOpen(true); // 24시간 이상이 지난 경우 모달을 표시
        }
    }, []);
    ...
    const handleDontShowAgain = () => {
        const currentTime = new Date().getTime();
        window.localStorage.setItem('modalClosedTime', currentTime); // 현재 시간을 localStorage에 저장
    ...
    return (
        <>
                    ...
                    >
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title animate__animated animate__rubberBand">
                                    공지사항
                                </h5>
                                <button
                                    type="button"
                            ...
                            <div className="modal-body">
                                <h5>
                                    해당 플랫폼은 학습 목적에 맞춰 다양한
                                    시스템과 프로세스를 체험할 수 있는 공간이며,
                                    <span style={{ color: 'red' }}>
                                        실제 결제나 예약과는 무관합니다.
                                    </span>
                                    사이트 내 모든 내용은 가상의 데이터로
                                    운영되고 있음을 안내드립니다.
                                </h5>
                                <div>
                                    <hr />
                                    <h5>로그인 정보</h5>
                                    <div>항공사1 : koreaair / Koreaair1!</div>
                                    <div>항공사2 : asiana / Asiana1!</div>
                                    항공사3 : jejuairline / Jeju111!
                                </div>
                                <div>회원1 : testuser1 / Testuser1!</div>
                                <div>회원1 : testuser2 / Testuser2!</div>
                                <div className="d-flex justify-content-between">
                                    <div>관리자1 : admin / Admin1234!</div>
                                    <div className="mx-3">
                                        <strong>관련문의 footer 확인</strong>
                                    </div>
                                </div>
                            </div>
            ...
            <header className="bg-dark" style={{ minWidth: '400px' }}>
                <div className="container">
                    {/* 가는편 오는편 기능 구현 */}
                    <div className="flight-all-div">
                        {' '}
                        {/* 전체 기능에 대한 div */}
                        <div
                            className="flight-search-check row mt-4 mb-4"
                            style={{ marginLeft: '7rem' }}
                        >
                            {' '}
                            {/* 안쪽 여백을 위한 div(전체 기능을 감싸는) */}
                            <h3 className="text-white mb-4 sparkle-move-text">
                                항공권 조회를 한 번에!
                            </h3>
                            ...
                            <div className="col-sm-3">
                                <button
                                    className="btn btn-primary upmove-text-hover"
                                    style={{ height: '5em' }}
                                    onClick={handleSearch}
                                    onFocus={handleInputFocus}
                                >
                                    <FaSearch className="icon" /> 검색하기
                                </button>
                            </div>
                        </div>
                        <div
                        ...
                        >
                            ...
                            {departureInputClick && ( // 출발지 입력창 클릭 시에만 보여주기
                                <>
                                    ...
                                    >
                                        <div className="col">
                                            ...
                                            {open === true || 화면} 오르쪽만 쓰겠다 */}
                                                {open === true && (
                                                    <ul
                                                        className="auto-search list-group"
                                                        style={{ width: '26%' }}
                                                    >
                                    ...
                                    {/* 검색창 */}
                                    <div
                                        className="flight-select-div mt-2 me-3 mb-3 bg-white"
                                        style={{ marginLeft: '-4%' }}
                                    >
                                        <div
                                                ...
                                                <ul className="list-group nation-group-box">
                                                    <li className="nation-list">
                                                        <button
                                                            ...
                                                            onClick={sendNationDep(
                                                                '한국',
                                                                departureNationalClick
                                                            )}
                                                        >
                                                            <span>한국</span>
                                                        </button>
                            ...
                            {destinationInputClick && ( // 도착지 입력창 클릭 시에만 보여주기
                                <>
                                    {/* 검색창 기능 */}
                                    <div
                                        className="flight-select-div mt-2 mb-3 bg-white"
                                                ...
                                                <ul className="list-group nation-group-box">
                                                    <li className="nation-list">
                                                        <button
                                                            ...
                                                            onClick={sendNationalDes(
                                                                '한국',
                                                                destinationNationalClick
                                                            )}
                                                        >
                                                            <span>한국</span>
                                                        </button>
                            ...
                            <div className="card border-0 mb-4 shadow-sm">
                                <div className="card-body bg-light rounded-3">
                                    ...
                                    <small className="form-text text-muted">
                                        {selectedCountry === 'jpy' &&
                                        exchangeRates.jpy
                                            ? `1 JPY = ${exchangeRates.jpy.toFixed(
                                                  3
                                              )} 원`
                                            : selectedCountry === 'vnd' &&
                                              exchangeRates.vnd
                                            ? `1 VND = ${exchangeRates.vnd.toFixed(
                                                  3
                                              )} 원`
                                            : selectedCountry === 'krw'
                                    ...
                                    {selectedCountry === 'jpy' &&
                                        exchangeRates.jpy && (
                                            ...
                                            >
                                                {`${amount.toLocaleString()} JPY는 ${(
                                                    amount *
                                                    exchangeRates.jpy.toFixed(3)
                                    ...
                                    {selectedCountry === 'vnd' &&
                                        exchangeRates.vnd && (
                                            ...
                                            >
                                                {`${amount.toLocaleString()} VND는 ${(
                                                    amount *
                                                    exchangeRates.vnd.toFixed(3)
                                                ).toLocaleString()} 원 입니다.`}
                                            </div>
                                        )}
                                </div>
                        ...
                        <div className="col-md-8">
                            <div className="card border-0 mb-4 shadow-sm">
                                <div className="card-body bg-light rounded-3">
                                    ...
                                    {weekWeather &&
                                    weekWeather.time &&
                                    weekWeather.time.length > 0 ? (
                                        <div className="row">
                                            {weekWeather.time.map(
                                                (date, index) => {
                                                    return (
                                                        <div
                                                            key={index}
